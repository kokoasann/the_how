...
bool Bullet::Start()
{
	...
	case K_uniBOM:
		...
		break;
	case uniBEEM:
		beemIN = NewGO<prefab::CEffect>(0,"effe");
...
void Bullet::Update()
{
	...
	case uniBOM:
		...
		for (int i = 0; i < 50; i++)
		{
			...
			QueryGOs<EnemyShip>("s", [&](EnemyShip* fo)->bool
			{
				if (fo != nullptr)
				{
					...
					for (auto qb : pd)
					{
						...
						if (d.Length() < 200.0f)
						{
							prefab::CEffect* e = NewGO < prefab::CEffect>(0,"effe");
	...
	case uniBEEM:
		if (beemIN->IsPlay())
		{
			...
			if (time >= 20 && beemin)
			{
				...
				beem = NewGO<prefab::CEffect>(0,"effe");
