...
void EnemyShipPR::Update()
{
	if (!Dead)
	{
		...
		case cross:
			...
			{
				FollingObjectPR* f = NewGO<FollingObjectPR>(0, "f");
				f->B = FollingObjectPR::BOMB::ShipFoll;
				f->pos = pos;
				time = 0;
			}
			pos += add;
		...
		case drop:
			...
			{
				EnemyShipPR* ene = NewGO<EnemyShipPR>(0, "s");
				ene->ship = MCBOM;
				ene->step = advance;
			...
			if (wait <= time && Random().GetRandDouble() <= 0.1)
			{
				...
				for (auto a : aaa)
				{
					FollingObjectPR* fo = NewGO<FollingObjectPR>(0, "s");
					fo->B = FollingObjectPR::BOMB::wing;
					CQuaternion rot = m_ship->GetRotation();
					CMatrix mTrans, mRot, mWorld;
		...
		case advance:
			...
			{
				FollingObjectPR* f = NewGO<FollingObjectPR>(0, "f");
				f->B = FollingObjectPR::BOMB::ShipFoll2;
				f->pos = pos;
	...
	else
	{
		...
		case Mdl_Calico:
			...
			{
				prefab::CEffect* ef = NewGO<prefab::CEffect>(0,"pe");
...
void EnemyShipPR::ShipBrain()
{

}

std::vector<CVector3> EnemyShipPR::Jud()
{
	switch (ship)
	{
	case Mdl_Calico:
