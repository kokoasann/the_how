...
namespace tkEngine{
	...
	public:
		...
		*/
		...
		/*!
		*@brief	ベクトルから新たな角度と新たなベクトルを得る。
		*内積から角度を求めて、外積からベクトルを求める。
		*方向転換の時とか便利かも。
		*正規化はせえへんでもええで。
		例：
		θ = vec.ThVe(oldV);
		Quat.SetRot(vec,θ);
		Quat.Multiply(oldQ,Quat);
		obj.SetRot(Quat)^^;
		*
		*/
		float ThetaVector(const CVector3& Av)
		{
			CVector3 v1, v2;
			v1.Set(vec);
			v1.Normalize();
		...
		*正規化はせえへんでもええで。
		*/
		float ThetaVector(const CVector3& Av1,const CVector3& Av2)
		{
			CVector3 v1, v2;
			v1 = Av1;
			v1.Normalize();
